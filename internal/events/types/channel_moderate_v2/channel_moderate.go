...
var triggerSupported = []string{"moderate"}
...
func (e Event) GenerateEvent(params events.MockEventParameters) (events.MockEventResponse, error) {
	...
	if params.ModerateAction == "" {
		params.ModerateAction = supportedActions[rand.Intn(len(supportedActions))]
	}
	...
	case models.TransportWebhook, models.TransportWebSocket:
		...
		eBody := models.ChannelModerateEventSubEvent{
			BroadcasterUserID:    params.ToUserID,
		...
		switch params.ModerateAction {
		case "vip":
			eBody.Action = "vip"
			eBody.Vip = &models.ChannelModerateVipAction{
		...
		case "unvip":
			eBody.Action = "unvip"
			eBody.Unvip = &models.ChannelModerateUnvipAction{
				UserID:    params.FromUserID,
				UserLogin: params.FromUserName,
				UserName:  params.FromUserName,
			}
		default:
			return events.MockEventResponse{}, fmt.Errorf("\"%s\" is not a valid moderate action, supported actions are %v", params.ModerateAction, supportedActions)
		}
		body.Event = eBody
